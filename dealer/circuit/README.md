...
- **Native**: used by the shuffle service (`rust/`) via `prove_from_decimal_salts` and `tree_from_decimal_salts` in `services.rs`.
...
**Why the root must be supplied externally**
...
This opens a concrete attack: the dealer pre-generates a valid proof for any deck, then sends players entirely different cards and salts. Players compute their root from the cards they received and call verify; since the proof isn't bound to their root, nothing mismatches and verification passes. The instance constraint is what makes this impossible.
